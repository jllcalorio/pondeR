...
#' (parametric or non-parametric) based on checks for normality and homogeneity of
...
#'   "lilliefors" (Lilliefors (K-S) test), or "auto" (uses Shapiro-Wilk for n < 50,
...
#' @param p_adjust_method P-value adjustment method for post-hoc comparisons: "holm", "hochberg",
...
#' @references Fox, J., & Weisberg, S. (2019). \emph{An R Companion to Applied Regression} (3rd ed.). Sage.
...
#' @references R Core Team (2023). \emph{R: A language and environment for statistical computing}. R Foundation for Statistical Computing, Vienna, Austria. URL \url{https://www.R-project.org/}.
#'
#' @examples
...
auto_compare <- function(data,
  ...
  if (test_type == "auto") {
    ...
    if (n_groups == 2) {
      for (g in groups) {
        group_data <- y[grp == g]
        norm_check <- check_normality(group_data, g, normality_method)

        assumptions <- rbind(assumptions, data.frame(
          check = norm_check$label,
          result = norm_check$method,
      ...
      # ANOVA: check normality of residuals
      fit <- lm(y ~ grp)
  ...
  if (perform_posthoc && n_groups > 2 && !is.null(test_result) && test_result$p.value < test_alpha) {
    if (requireNamespace("rstatix", quietly = TRUE)) {
      posthoc_result <- tryCatch({
        ...
        # Different tests return different column names - standardize them
        # Tukey and Games-Howell return: group1, group2, p.adj
        # Dunn returns: group1, group2, p, p.adj
        ...
        names(ph)[names(ph) %in% c("mean", "median")] <- paste0(c("mean", "median"), "1")

        # Merge with group2
        ph <- merge(ph, summary_stats, by.x = "group2", by.y = "group", all.x = TRUE)
        names(ph)[names(ph) %in% c("mean", "median")] <- paste0(c("mean", "median"), "2")
  ...
  if (verbose) {
    ...
    if (!is.null(posthoc_result)) {
      ...
      sig_comparisons <- sum(posthoc_result$significant)
...
print.auto_compare <- function(x, ...) {
  ...
  if (!is.null(x$posthoc_result)) {
    ...
    sig_only <- x$posthoc_result[x$posthoc_result$significant,
...
summary.auto_compare <- function(object, ...) {
  ...
  if (!is.null(object$posthoc_result)) {
    ...
    cols_to_show <- c("group1", "group2")

    # Add p-value columns if they exist
    if ("p" %in% names(object$posthoc_result)) cols_to_show <- c(cols_to_show, "p")
    if ("p.adj" %in% names(object$posthoc_result)) cols_to_show <- c(cols_to_show, "p.adj")
    if ("p.adj.signif" %in% names(object$posthoc_result)) cols_to_show <- c(cols_to_show, "p.adj.signif")
    if ("interpretation" %in% names(object$posthoc_result)) cols_to_show <- c(cols_to_show, "interpretation")
...
#' Plot method for auto_compare objects
#' @param x An object of class "auto_compare".
#' @param ... Further arguments passed to or from other methods.
#' @export
plot.auto_compare <- function(x, ...) {
  # Determine number of plots
  n_plots <- 2
  if (!is.null(x$posthoc_result) && nrow(x$posthoc_result) > 0) {
    n_plots <- 3
  }
  ...
  # Plot 1: Boxplot with means
  boxplot(outcome ~ group, data = x$raw_data,
          main = "Comparison of Groups",
          xlab = x$group_var,
          ylab = x$outcome,
          col = "lightblue")
  means <- tapply(x$raw_data$outcome, x$raw_data$group, mean)
  points(1:length(means), means, pch = 19, col = "red", cex = 1.5)
  legend("topright", legend = "Mean", pch = 19, col = "red", bty = "n")

  # Add significance annotation
  if (x$test_result$p.value < 0.001) {
    p_label <- "p < 0.001 ***"
  } else if (x$test_result$p.value < 0.01) {
    p_label <- sprintf("p = %.3f **", x$test_result$p.value)
  } else if (x$test_result$p.value < 0.05) {
    p_label <- sprintf("p = %.3f *", x$test_result$p.value)
  } else {
    p_label <- sprintf("p = %.3f ns", x$test_result$p.value)
  }
  mtext(p_label, side = 3, line = 0.5, cex = 0.8)

  # Plot 2: Q-Q plot for normality check
  if (x$parametric && length(levels(x$raw_data$group)) > 2) {
    # Use the stored fitted model if available, otherwise create it
    if (!is.null(x$parameters$paired) && x$parameters$paired) {
      # For paired tests, Q-Q plot of differences
      grp <- x$raw_data$group
      y <- x$raw_data$outcome
      diffs <- y[grp == levels(grp)[2]] - y[grp == levels(grp)[1]]
      qqnorm(diffs, main = "Q-Q Plot of Differences")
      qqline(diffs, col = "red", lwd = 2)
    } else if (!is.null(x$parametric) && x$parametric && length(levels(x$raw_data$group)) > 2) {
      qqnorm(lm(outcome ~ group, data = x$raw_data)$residuals, main = "Q-Q Plot of Residuals")
      qqline(lm(outcome ~ group, data = x$raw_data)$residuals, col = "red", lwd = 2)
    } else {
      qqnorm(x$raw_data$outcome, main = "Q-Q Plot of Outcome")
      qqline(x$raw_data$outcome, col = "red", lwd = 2)
    }
  } else {
    qqnorm(x$raw_data$outcome, main = "Q-Q Plot of Outcome")
    qqline(x$raw_data$outcome, col = "red", lwd = 2)
  ...
  # Plot 3: Post-hoc comparison matrix (if applicable)
  if (!is.null(x$posthoc_result) && nrow(x$posthoc_result) > 0) {
    # Create a matrix of p-values
    groups <- levels(x$raw_data$group)
    n_groups <- length(groups)
    p_matrix <- matrix(1, nrow = n_groups, ncol = n_groups)
    rownames(p_matrix) <- colnames(p_matrix) <- groups

    for (i in 1:nrow(x$posthoc_result)) {
      g1 <- as.character(x$posthoc_result$group1[i])
      g2 <- as.character(x$posthoc_result$group2[i])
      p_val <- x$posthoc_result$p.adj[i]
      p_matrix[g1, g2] <- p_val
      p_matrix[g2, g1] <- p_val
    }

    # Convert to significance levels
    sig_matrix <- ifelse(p_matrix < 0.001, 3,
                         ifelse(p_matrix < 0.01, 2,
                                ifelse(p_matrix < 0.05, 1, 0)))

    # Plot heatmap
    image(1:n_groups, 1:n_groups, sig_matrix,
          col = c("white", "#fee5d9", "#fc9272", "#de2d26"),
          xlab = "", ylab = "", main = "Post-Hoc Significance",
          axes = FALSE)
    axis(1, at = 1:n_groups, labels = groups, las = 2)
    axis(2, at = 1:n_groups, labels = groups, las = 1)

    # Add significance symbols
    for (i in 1:n_groups) {
      for (j in 1:n_groups) {
        if (i != j) {
          sig_label <- if (sig_matrix[i, j] == 3) "***"
          else if (sig_matrix[i, j] == 2) "**"
          else if (sig_matrix[i, j] == 1) "*"
          else "ns"
          text(j, i, sig_label, cex = 0.8)
        }
      }
    }
  }

  mtext(sprintf("Auto-Comparison: %s by %s", x$outcome, x$group_var),
